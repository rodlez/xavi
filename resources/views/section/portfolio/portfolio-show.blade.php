    ...
    <div class="border-2 border-black rounded-lg p-4">
        ...
        <div class="columns-1 gap-5 sm:columns-2 sm:gap-2 lg:columns-3 xl:columns-4 [&>img:not(:first-child)]:mt-8 p-0">
            ...
            @foreach ($files as $file)
                ...
                @if ($file->type == 'image')
                    <!-- Get the possible translations for the image to show on the gallery -->
                    @php
                        $imgTitle = '';
                        $imgDescription = '';
                    @endphp
                    ...
                    @foreach ($file->translations as $translation)
                        @if ($translation->lang_id == $languageId)
                            @php
                                $imgTitle = $translation->title;
                                $imgDescription = $translation->description;
                            @endphp
                        @endif
                    @endforeach
                    ...
                    <div class="flex flex-col py-2">
                        <a href="{{ asset('storage/' . $file->path) }}" data-lightbox="roadtrip"
                            data-title="{{ $imgTitle != '' ? $imgTitle : $file->original_filename }}">
                            <img class="h-auto max-w-full rounded-sm object-cover object-center"
                                src="{{ asset('storage/' . $file->path) }}" alt="gallery-photo" />
                        </a>
                        {{-- <h3>{{($imgDescription != "") ? $imgDescription : ""}}</h3>  --}}
                    </div>
                @endif
                {{-- </div> --}}
            @endforeach
            ...
            @foreach ($files as $file)
                ...
                @if ($file->type == 'document')
                    <div class="flex flex-row justify-start gap-2 py-0">
                        ...
                        <div class="flex"><a href="{{ asset('storage/' . $file->path) }}"
                                title="{{ __('generic.open') }} PDF {{ $file->original_filename }}" target="_blank">
