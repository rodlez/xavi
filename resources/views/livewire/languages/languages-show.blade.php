<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500">
        ...
        <a href="/languages/{{$language->id}}" class="font-bold text-black border-b-2 border-b-yellow-600">Info</a>
    </div>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="flex flex-row justify-between items-center py-4 bg-yellow-400">
            <div>
                <span class="text-lg text-white px-4">Language Info</span>
            </div>
        </div>
        ...
        <div class="mx-auto w-11/12 py-4 px-2">
            ...
            <div class="flex flex-row justify-start items-center gap-4 py-2">
                <!-- Name -->
                <input type="text" id="name" class="bg-zinc-200 border border-zinc-300 text-gray-900 text-md rounded-lg w-full sm:w-1/2 pl-2 p-2  dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-yellow-500 dark:focus:border-yellow-500" value="{{ $language->name }}" disabled>
                <!-- Code -->
                <input type="text" id="code" class="bg-zinc-200 border border-zinc-300 text-gray-900 text-md rounded-lg w-full sm:w-1/2 pl-2 p-2  dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-yellow-500 dark:focus:border-yellow-500" value="{{ $language->code }}" disabled>
                <!-- Edit -->
                ...
                <form action="{{ route('languages.destroy', $language) }}" method="POST">
                    ...
                    @method('DELETE')
                    <button onclick="return confirm('Are you sure you want to delete the language: {{ $language->name }}?')">
