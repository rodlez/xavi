<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500 capitalize">
        <a href="/pf_tags_trans" class="font-bold text-black {{$underlineMenuHeader}}">{{__("admin/portfolio/portfolioTags.menuIndexTranslations")}}
        </a>
    </div>
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <div class="flex flex-row justify-between items-center py-4 {{$bgMenuColor}}">
                <div>
                    <span class="text-lg text-white capitalize px-4">{{__("generic.tags")}} <span class="text-sm">({{ $totalEntries }})</span>
                    {{__("generic.translations")}} <span class="text-sm">({{ $totalTranslations }})</span>
                    </span>
                </div>
                <div class="px-4">
                    {{-- <a href="{{ route('pf_tags_trans.create') }}"
                        ...
                        <table class="min-w-full ">
                            <thead>
                                <tr class="text-black text-left text-sm font-normal uppercase">
                                    <th></th>
                                    <th wire:click="sorting('pf_tags_trans.id')" scope="col"
                                        class="p-2 hover:cursor-pointer hover:{{$menuTextColor}} {{ $column == 'pf_tags_trans.id' ? $menuTextColor : '' }}">
                                        id {!! $sortLink !!}</th>
                                    <th wire:click="sorting('pf_tags_trans.pf_tag_id')" scope="col"
                                        class="p-2 hover:cursor-pointer hover:{{$menuTextColor}} {{ $column == 'pf_tags_trans.pf_tag_id' ? $menuTextColor : '' }}">
                                        {{__("generic.tag")}} {!! $sortLink !!}</th>
                                    ...
                                    <th wire:click="sorting('pf_tags_trans.created_at')" scope="col"
                                        class="p-2 hover:cursor-pointer hover:{{$menuTextColor}} {{ $column == 'pf_tags_trans.created_at' ? $menuTextColor : '' }}">
                                        {{__("generic.created")}} {!! $sortLink !!}</th>
                                    <th wire:click="sorting('pf_tags_trans.updated_at')" scope="col"
                                        class="p-2 hover:cursor-pointer hover:{{$menuTextColor}} {{ $column == 'pf_tags_trans.updated_at' ? $menuTextColor : '' }}">
                            ...
                            <tbody>
                                ...
                                @foreach ($translations as $translation)
                                    <tr
                                        ...
                                        <td class="p-2">{{ $translation->tag->name }}</td>
                                        <td class="p-2">{{ $translation->language->code }}</td>
                                        <td class="p-2">{{-- <a
                                                href="{{ route('pf_tags_trans.show', $translation) }}">{{ $translation->name }}</a> --}}
                                            {{ $translation->name }}
                                        </td>
