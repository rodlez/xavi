<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500 capitalize">
        ...
        /
        <a href="{{route('pf_tags_trans')}}" class="text-black {{ $textMenuHeader }}">{{ __('generic.translations') }}
        </a>
    </div>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="flex flex-row justify-between items-center py-4 {{ $bgMenuColor }}">
            <div>
                <span class="text-lg text-white capitalize px-4">{{ __('admin/portfolio/portfolioTags.titleHeader') }}
                    <span class="text-sm">({{ $search != '' ? $found : $total }})</span></span>
            </div>
            <div class="px-4">
                <a href="{{ route('pf_tags.create') }}"
                    ...
                    <table class="min-w-full ">
                        <thead>
                            <tr class="text-black text-left text-sm font-normal uppercase">
                                <th></th>
                                <th wire:click="sorting('pf_tags.id')" scope="col"
                                    class="p-2 hover:cursor-pointer hover:{{ $menuTextColor }} {{ $column == 'pf_tags.id' ? $menuTextColor : '' }}">
                                    id {!! $sortLink !!}</th>
                                ...
                                <th wire:click="sorting('pf_tags.created_at')" scope="col"
                                    class="p-2 hover:cursor-pointer hover:{{ $menuTextColor }} {{ $column == 'pf_tags.created_at' ? $menuTextColor : '' }}">
                                    {{ __('generic.created') }} {!! $sortLink !!}</th>
                                <th wire:click="sorting('pf_tags.updated_at')" scope="col"
                                    class="p-2 hover:cursor-pointer hover:{{ $menuTextColor }} {{ $column == 'pf_tags.updated_at' ? $menuTextColor : '' }}">
                        ...
                        <tbody>
                            ...
                            @foreach ($tags as $tag)
                                <tr
                                    ...
                                    <td class="p-2"><a
                                            href="{{ route('pf_tags.show', $tag) }}">{{ $tag->name }}</a>
                                    </td>
                                    <td class="p-2">
                                        {{ strlen($tag->description) >= 20 ? substr($tag->description, 0, 20) . '...' : $tag->description }}
                                    </td>
