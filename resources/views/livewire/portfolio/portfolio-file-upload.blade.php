<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <!-- Upload Form -->    
        <div class="mx-4 my-4 py-4 px-2 bg-gray-100 rounded">
            <form wire:submit.prevent="save">
                <label class="text-lg text-gray-500 font-semibold mb-2 block">{{ __('generic.uploadFiles') }} <span
                        class="text-xs font-bold">({{ __('generic.maximum') }} 3)</span></label>
                ...
                @if (count($files) > 3)                    
                    <div class="flex flex-col my-2 p-4 bg-red-200 rounded-lg">
                        ...
                        <span class="text-sm text-red-600 font-bold">(3) {{ __('generic.maxFiles') }}</span>
                    </div>
                @else
                    <button
        ...
        <div class="mx-4 mb-12">
            @if (count($files) !== 0)
                <div class="py-2">
                    <span class="text-lg text-gray-500 font-semibold mb-4">{{ __('generic.filesSelected') }}</span>
                    <span class="text-sm {{ ((count($files) <= 3) ? 'text-green-600' : 'text-red-600')}}">
                        ({{ count($files) }})
                    </span>
                </div>
