<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="flex flex-row justify-between items-center py-4 {{ $bgMenuColor }}">
            <div>
                <span
                    class="text-lg text-white capitalize px-4">{{ __('admin/portfolio/portfolioCategories.titleHeader') }}
                    <span class="text-sm">({{ $search != '' ? $found : $total }})</span></span>
            </div>
            <div class="px-4">
                <a href="{{ route('pf_categories.create') }}"
                    class="text-white text-sm capitalize sm:text-md rounded-lg py-2 px-4 bg-black hover:bg-gray-600 transition duration-1000 ease-in-out">{{ __('generic.new') }}
                </a>
            </div>
        </div>
        <!-- Search -->
                                ...
                                <tr
                                    ...
                                    <td class="p-2 text-center normal-case">
                                        ...
                                        @foreach ($this->translationLinks($category) as $translation)
                                            ...
                                            @else
                                                <a href="{{ route('pf_categories_trans.create', ['category' => $category, 'missingTranslationId' => $translation['langId']]) }}"
