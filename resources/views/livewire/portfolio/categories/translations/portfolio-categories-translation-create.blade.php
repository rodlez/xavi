<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500">
        ...
        <a href="/pf_categories/{{ $category->id }}" class="hover:text-violet-600">{{ $category->name }}</a> /
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div>
            ...
            <div class="mx-auto w-11/12 py-4 px-2">
                ...
                @if ($isTranslated == false)
                    ...
                    <form wire:submit="save">
                        ...
                        @if (!$translationLanguage)
                            ...
                            <h2 class="text-lg font-bold pt-2 pb-1 px-2">Language <span class="text-red-600">*</span>
                            </h2>
                            <div class="relative">
                                <select wire:model.live="language_id" name="language_id" id="language_id"
                                    ...
                                    @foreach ($languages as $language)
                                        <option value="{{ $language->id }}" class="text-green-600"
                                            @if (old('language_id') == $language->id) selected @endif>{{ $language->name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                @else
                    <h3 class="text-red-600">Translation for this language ({{$translationLanguage->name}}) already created.</h3>
                    <a href="{{ route('pf_categories.show', $category) }}" class="font-bold">Go Back</a>
                @endif
            </div>
