<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500 capitalize">
        <a href="{{ route('portfolios') }}"
            class="{{ $textMenuHeader }}">{{ __('admin/portfolio/portfolio.menuIndex') }}</a> /
        <a href="{{ route('portfolios.show', $portfolio) }}" class="{{ $textMenuHeader }}">{{ $portfolio->name }}</a> /
        <a href="{{ route('portfolios_trans.create', ['portfolio' => $portfolio, 'missingTranslationId' => $translationLanguage->id]) }}"
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="mx-auto w-11/12 my-4 px-2">
            ...
            @if ($isTranslated == false)
                ...
                <!-- Create Translation Portfolio Message -->
                <div
                    ...
                    <form wire:submit="save">
                        ...
                        @if ($errors->any())
                            <div class="bg-red-400 p-2 rounded-md my-2">
                                <span
                                    class="text-sm sm:text-lg text-white font-bold">{{ __('generic.errorForm') }}</span>
                            </div>
                        @endif
                                ...
                                <select wire:model.live="pf_type_trans_id" name="pf_type_trans_id" id="pf_type_trans_id"
                                    ...
                                    @foreach ($types as $type)
                                        <option value="{{ $type->id }}" class="text-green-600"
                                            @if (old('pf_type_trans_id') == $type->id) selected @endif>{{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
                                <div class="absolute flex items-center inset-y-0 left-0 pointer-events-none">
                                ...
                                <select wire:model="selectedTags" name="selectedTags" id="selectedTags" multiple
                                    ...
                                    @foreach ($tags as $tag)
                                        <option value="{{ $tag->id }}" class="text-green-600"
                                            @if (old('selectedTags') == $tag->id) selected @endif>{{ $tag->name }}
                                        </option>
                                    @endforeach
                                </select>
                                <div class="absolute flex items-center inset-y-0 left-0 pointer-events-none">
