<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500 capitalize">
        ...
        /
        <a href="/portfolios/{{ $file->portfolio->id }}/file/{{ $file->id }}/translation/{{ $translation->id }}/edit"
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="flex flex-row py-4 {{ $bgMenuColor }}">
            <span class="text-lg text-white px-4 capitalize">{{ __('generic.portfolio') }} {{ __('generic.image') }}
                {{ __('generic.translation') }} {{__('generic.edit')}}</span>
        </div>
                ...
                <!-- TRANSLATION -->
                    <div class="flex flex-col my-8">
                        ...
                        <div class="flex flex-col text-black capitalize bg-gray-200 rounded-b-lg sm:rounded-tr-lg">
                            ...
                            <span
                                class="{{ $translationName }} p-2 rounded-b-lg">{{ $translation->language->name }}</span>
                        </div>
                    </div>

                    <!-- Edit Translation Type Message -->
                    <div
                        class="flex w-full sm:w-fit text-lg {{ $editTranslation }} font-light normal-case rounded-t-md p-2">
                    ...
                    <div class="bg-slate-200 rounded-b-md my-0 p-2">
                        <form action="{{ route('portfoliosfile_trans.update', [$portfolio, $file, $translation]) }}" method="POST">
                            ...
                            <div class="relative">
                                <input wire:model="title" name="title" id="title" type="text"
                                    value="{{ $translation->title }}" maxlength="100"
