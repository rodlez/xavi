<div class="max-w-7xl mx-auto sm:pb-8 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-row justify-start items-start gap-1 text-sm py-3 px-4 text-slate-500 capitalize">
        ...
        <a href="/portfolios/{{ $portfolio->id }}" class="font-bold text-black {{$underlineMenuHeader}}">{{__("generic.info")}}</a>
    </div>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="flex flex-row justify-between items-center py-4 {{$bgMenuColor}}">
            <div>
                <span class="text-lg text-white capitalize px-4">{{__("admin/portfolio/portfolio.titleHeader")}} - {{ $portfolio->name }}</span>
            </div>
        </div>
